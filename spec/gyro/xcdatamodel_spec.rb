...
module Gyro
  describe Parsers::XCDataModel do
    before do
      Gyro::Log.quiet = true
    end
    ...
    it 'check raise an error for file' do
      ...
      expect { Parsers::XCDataModel::XCDataModel.new(xcdatamodel_dir) }.to raise_error "\e[1;31m! Unable to find contents of xcdatamodel dir\e[0m"
    end
    ...
    it 'check raising relationship error' do
      ...
      expect { Parsers::XCDataModel::XCDataModel.new(xcdatamodel_dir) }.to raise_error "\e[1;31m! The relationship \"user\" from \"FidelityCard\" is wrong - please fix it\e[0m"
    end
    ...
    it 'check raising undefined type error' do
      ...
      expect { Parsers::XCDataModel::XCDataModel.new(xcdatamodel_dir) }.to raise_error "\e[1;31m! The attribute \"name\" from \"Product\" has no type - please fix it\e[0m"
    end
    ...
    it 'check raising enum type error' do
      ...
      expect { Parsers::XCDataModel::XCDataModel.new(xcdatamodel_dir) }.to raise_error "\e[1;31m! The attribute \"type\" from \"Shop\" is enum with incorrect type (not Integer) - please fix it\e[0m"
    end
