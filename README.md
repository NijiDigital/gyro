...
The `.xcdatamodel` is a a file used to represent Core Data entities. It can be created or edited with a graphical user interface in Xcode.

But although the initial aim of the `.xcdatamodel` file is to create a Core Data model, with DBGenerator you will now be able to use it to create a [Realm.io](https://realm.io) model files as well. 
...
The `.xcdatamodel` Xcode editor allows you to add 'user infos' to your entities, attributes or relationships. Each 'user info' is a couple key/value.

With the help of these 'user infos', you will be able to give DBGenerator extra information about your model classes. For example, you can tell which attribute is the primary key, the attributes to ignore, the JSON correspondances,...
